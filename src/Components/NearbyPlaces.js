...
//component imports
import { editNearbyPlaces } from "../store";
...
const NearbyPlaces = () => {
  ...
  const openNow = (category) => {
    // for seeded data
    let placesWithHours = nearbyPlaces
  ...
  const openSoon = (timeQueried, category) => {
    const placesWithHours = nearbyPlaces
      .filter((place) => place.opening_hours)
      .filter((place) => place.types.includes(category));
    const today = googleDate();
    const time = googleTime();
  ...
  const saveNotesAndRating = (place, auth) => {
    console.log(place, rating, notes, auth);
    dispatch(editNearbyPlaces(place, rating, notes, auth));
  };
  ...
  return (
    <>
      <div>
        <div id="welcomePage">
          {capitalizeFirstLetter(auth.username || "")},{" "}
          {openNow(categoryName).length}{" "}
          ...
          <div id="welcomeSmaller">
            {openSoon(30, categoryName).length === 0
              ? ""
              : `${openSoon(30, categoryName).length} more within 30 minutes.${(
                  <br />
                )}`}
            {closingSoon(120, categoryName).length === 0
              ? ""
              : `${
                  closingSoon(120, categoryName).length
                } will close within two hours.`}
            <br />
          </div>
        </div>
            ...
            return (
              <Button
                ...
                sx={{
                  ...
                  mx: 0.4,
                  backgroundColor:
                    selectedCategory === category ? "#1C5D99;" : "transparent",
                  color: selectedCategory === category ? "white" : "black",
                  "&:hover": {
                    color: "white",
                    backgroundColor:
                      selectedCategory === category ? "#1C5D99;" : "#A2A3BB",
                  },
                }}
                    ...
                    >
                      ...
                      <CardContent>
                        ...
                        >
                          ...
                          <br />
                          Google Rating: {place.rating} with{" "}
                      ...
                      >
                        ...
                        >
                          <Box>
                            ...
                            <ul
                              style={{ paddingLeft: "8px", fontSize: "1.1rem" }}
                            >
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[0]}
                              </li>
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[1]}
                              </li>
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[2]}
                              </li>
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[3]}
                              </li>
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[4]}
                              </li>
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[5]}
                              </li>
                              <li style={{ listStyleType: "none" }}>
                                {place.weekday_text[6]}
                              </li>
                            </ul>
                          </Box>
                          <Box>
